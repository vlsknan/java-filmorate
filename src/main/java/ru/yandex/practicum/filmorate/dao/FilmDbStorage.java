...
////                    film.getDuration(), film.getMpa(), film.getId()) == 0 ?
////                    Optional.empty() :
////                    Optional.of(film);
//            KeyHolder keyHolder = new GeneratedKeyHolder();
//            jdbcTemplate.update(connection -> {
...
//        final String sqlQuery = "select distinct F.FILM_ID, F.FILM_NAME, F.DESCRIPTION, " +
//                "F.RELEASE_DATE, F.DURATION, M.MPA_ID, M.MPA_NAME, G.GENRE_ID " +
//                "from FILMS F " +
//                "join MPA M on M.MPA_ID = F.MPA_ID " +
//                "join FILMS_GENRES G on F.FILM_ID = G.FILM_ID " +
//                "where F.FILM_ID in (select F.FILM_ID from FILMS F " +
